		...
		if (session_state == GGMR_SESSION_STATE.start) 
			...
			if (session_local_client_type == GGMR_CLIENT_TYPE.player)
				...
				if (session_start_timer == 0) 
					...
					script_execute(session_callbacks.save_game, session_frames[@ GGMR_RELATIVE_FRAME][@ GGMR_FRAME.game_state]);	
		...
		#region NORMAL
			...
			if (session_local_client_type == GGMR_CLIENT_TYPE.player)
				...
				repeat (_number_of_frames)
					...
					//Get Local Input
					ggmr_session_input_add_local(_first_iteration);
					...
					script_execute(session_callbacks.advance_frame, session_frames[@ GGMR_RELATIVE_FRAME][@ GGMR_FRAME.inputs], GGMR_RELATIVE_FRAME);
					//*/
