		...
		if (is_launched() || state == PLAYER_STATE.tumble)
			...
			if (_ledge != noone)
				...
				if (sign(_ledge.x - x) == _ledge.image_xscale)
					...
					if (attack_can_start(shelly_airspecial))
						{
						_lx = _center;
			...
			if ((state == PLAYER_STATE.attacking && attack_script == shelly_airspecial) || (cpu_check_input(INPUT.special) && !on_ground()))
			{
					_lx = _center;
			}
		}
