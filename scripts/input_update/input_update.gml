...
function input_update()
	...
	//Dual stick angle
	if (scs[@ SCS.dual_stick_angle])
		{
		//Reuse code from <calculate_di_direction>
		//Set a temporary placeholder variable, which holds the stick direction angle if the dir is always zero
		var _lstick_dist = point_distance(0, 0, _lx, _ly);
		var _rstick_dist = point_distance(0, 0, _rx, _ry);
		if (abs(_lstick_dist) > stick_tilt_amount && abs(_rstick_dist) > stick_tilt_amount)
			{
			var _lstick_dir = point_direction(0, 0, _lx, _ly);
			var _rstick_dir = point_direction(0, 0, _rx, _ry);
			var _temp = (_rstick_dir - _lstick_dir);
			...
			_lstick_dir += (angle_difference(_temp, 0) / 90) * 22.5;
			_lx = lengthdir_x(_lstick_dist, _lstick_dir);
			_ly = lengthdir_y(_lstick_dist, _lstick_dir);
			}
		}
	
	//Cache final stick values
	stick_cache_values
		(
