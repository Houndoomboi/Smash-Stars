...
function game_local_input_write()
	...
	if (_dt == DEVICE.keyboard)
			...
			if (_input < INPUT.LENGTH)
				...
				switch (_input)
					...
					case CC_INPUT_KEYBOARD.short_hop:
						...
						break;
					case CC_INPUT_KEYBOARD.angle:
						if (keyboard_check(_key)) then _angle_key_held = true;
						break;
		...
		if (_angle_key_held)
			{
			var _direction = point_direction(0, 0, _lx, _ly) - 22.5;
			_lx = lengthdir_x(1, _direction);
			_ly = lengthdir_y(1, _direction);
			}
		}
	#endregion 
	#region Controller
			...
			if (_input < INPUT.LENGTH)
				...
				switch (_input)
					...
					case CC_INPUT_CONTROLLER.short_hop:
						...
						break;
					case CC_INPUT_CONTROLLER.angle:
						// Snap angle to nearest 8 degree angle
						if (controller_check_input(_d, _control, false))
							{
							_lx = (abs(_lx) > stick_tilt_amount) ? sign(_lx) : 0;
							_ly = (abs(_ly) > stick_tilt_amount) ? sign(_ly) : 0;
							}
						break;
					default: crash("[game_local_input_write] Invalid controller input (", _input, ")");
					}
				}
			}
		}
