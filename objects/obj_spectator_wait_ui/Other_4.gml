	...
	function()
		...
		switch (_struct.packet_type)
			...
			case "ready_up_data":
				...
				_player_number = _struct.player_number;
				_player = engine().css_player_data[@ _player_number];
				_player[@ CSS_PLAYER.character] = _struct.character;
				_player[@ CSS_PLAYER.color] = _struct.color;
				_cc = custom_controls_create();
				_cc.right_stick_input = _struct.right_stick_input;
				_cc.scs = _struct.scs;
				_cc.acs = _struct.acs;
				
				//Profile
				_opponent_name = _struct.name;
				...
				_connection = ggmr_net_connection_find(ggmr_net_ip_address_convert(_async_load[? "ip"]), _async_load[? "port"]);
				with (obj_ui_image)
					{
			...
			case "start_data":
				...
				for (var i = 0; i < _num; i++)
					...
					if (i == obj_spectator_wait_ui.local_player_number) then continue;
					
					var _data = _css_data[@ i];
					_player = engine().css_player_data[@ i];
