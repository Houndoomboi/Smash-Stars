		...
		switch (_type) 
			...
			case GGMR_PACKET_TYPE.join:
				...
				if (is_lobby_leader) 
					...
					for (i = 0; i < ds_list_size(lobby_join_requests); i++) 
						{
			...
			case GGMR_PACKET_TYPE.sync:
				...
				if (ggmr_lobby_equals(_sending_ip))	
					...
					for (i = 0; i < _lobby_size; i++) 
						{
						var _name = buffer_read(_buff, buffer_string);
						_port = buffer_read(_buff, buffer_u16);
						_ip = buffer_read(_buff, buffer_string);
						_type = buffer_read(_buff, buffer_u8);
