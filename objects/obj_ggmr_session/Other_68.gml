	...
	if (buffer_exists(_buff)) 
		...
		switch (_type) 
			...
			case GGMR_PACKET_TYPE.input:
				//Get the packet data
				_number = buffer_read(_buff, buffer_u32);
				...
				if (session_desync_data.frame == _frame)
					...
					if (session_desync_data.value != _value)
						...
						for (var i = 0; i < array_length(session_clients_remote); i++) 
							...
							_connection = session_client_list[| _num][@ GGMR_CLIENT.connection];
						...
						script_execute(session_callbacks.on_event, GGMR_EVENT.desync_detected, _msg);
						}
					}
			break;
			...
			case GGMR_PACKET_TYPE.input_delay_request:
				...
				_connection = ggmr_net_connection_find(_ip, _port);
				if (_connection != -1)
					{
					...
					for (var i = 0; i < array_length(session_clients_remote); i++)
						{
						_number = session_clients_remote[@ i];
